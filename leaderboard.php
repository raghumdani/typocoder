		...
		<div class = "main_container">
			<?php
			  ...
			  include ("Util/all_colors.php");
				...
				printf("</tr>");
				$sql = "SELECT DISTINCT user_name FROM submissions WHERE cid = $cid and practice = 'no';";
				$result = mysqli_query($conn, $sql);
				if(!$result) {
					header("Location: index.php");
					exit(0);
				...
				while($row = mysqli_fetch_assoc($result)) {
					$uu = $row["user_name"];
					$sql = "SELECT * FROM submissions WHERE user_name = '$uu' and cid = $cid and practice = 'no';";
					$obj = new User();
					$obj->user = $uu;
					$obj->que = array();
					$obj->quen = array();
				...
				foreach($ranklist as $a) {
					...
					while($row = mysqli_fetch_assoc($result)) {
						...
						if(isset($a->quen[$qq])) {
							...
							$RJ_Verdict = isset($a->RJ_Verdict[$qq]) ? $a->RJ_Verdict[$qq] : "Rejected";
							$verdict = $RJ_Verdict;
							$color = color_of_verdict($RJ_Verdict);
							$message = "<b><a style = 'color: ".$color.";' href = 'display_source.php?cid=$cid&sub_id=$sub_id' title = 'View source' target = '_blank'>".$RJ_Verdict."</a></b>";
						}
						if(isset($a->que[$qq])) {
							$sub_id = $a->sub_idAC[$qq];
							$time_taken = $a->que[$qq];
							$verdict = 'Accepted';
							$message = "<b><a style = 'color: green;' href = 'display_source.php?cid=$cid&sub_id=$sub_id' title = 'View source' target = '_blank'>Accepted</a></b>";
						}
						if($message == "<b>Unattempted</b>") {
							printf("<td>%s</td>", $message);
						} else {
							printf("<td>%s (<span style = 'text-align: center; color: red;'>%d</span>)", $message, $a->RJcnt[$qq]);
						}
					...
					printf("<td><a href = 'user_submissions.php?cid=$cid&user=$a->user' title = 'View all submissions' target = '_blank'>View</a></td>");
					printf("</tr>");
				}
				printf("</table>");
				printf("</div>");
