				...
				foreach($ranklist as $a) {
					...
					while($row = mysqli_fetch_assoc($result)) {
						...
						if(isset($a->quen[$qq])) {
							...
							$message = "<b><a style = 'color: red;' href = 'display_source.php?cid=$cid&sub_id=$sub_id' title = 'View source' target = '_blank'>Rejected</a></b>";
						}
						if(isset($a->que[$qq])) {
					...
					printf("<td><a href = 'user_submissions.php?cid=$cid&user=$a->user' title = 'View all submissions'>View</a></td>");
					printf("</tr>");
				}
				printf("</table>");
				printf("</div>");
