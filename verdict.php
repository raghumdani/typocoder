	...
	<body>
		...
		<div class = "main-container">
			<?php
				...
				if(!$isAdmin) {
				  if ($practice == 'no') {
				    $sql = "SELECT COUNT(*) FROM submissions WHERE user_name = '$user_name' and qid = $qid and cid = $cid and practice = '$practice';";
				    $result = mysqli_query($conn, $sql);
					  if(!$result) {
						  mysqli_close($conn);
						  die("Some_Error");
					  }
				    $submissions_done = mysqli_fetch_array($result)[0];
				    settype($submissions_done, 'integer');
				    if ($submissions_done > 4) {
				      printf("<h1>You have exceeded submissions limit on this problem. Your best submission will be considered after the end of the contest</h1>");
				    } else {
				      $total_left = 5 - $submissions_done - 1;
				    }
				  }
				  if ($practice == 'yes' || $total_left > -1) {
				    $sql = "INSERT INTO submissions (qid, cid, user_name, practice, time_limit) VALUES($qid, $cid, '$user_name', '$practice', $time_limit);";
				    $result = mysqli_query($conn, $sql);
				    if(!$result) {
					    mysqli_close($conn);
					    die("Some_Error");
				...
				if ($isAdmin || $practice == 'yes' || $total_left > -1) {
        ...
        if ($isAdmin || $practice == 'yes' || $total_left > -1) {
			    ...
			    if ($verdict != 'Accepted' && $verdict != 'Rejected') {
				    printf("<h1>%s</h1>", $verdict);
				    if ($practice == 'no') printf("<h1>Total tries left : %d</h1>", $total_left);
			    }
			    printf("</div>");
			  }
			  if ($verdict == 'Accepted' || $verdict == 'Rejected')
